...
while True:
    ...
    if positions_file.lower() == 'y':
        # 從文本文件中讀取 JSON 數據
        with open('./potsition_data/keyboard_pos_data.txt', 'r') as file:
            keyboard_positions = json.load(file)

        with open('./potsition_data/rgb_pos_data.txt', 'r') as file:
    ...
    if positions_file.lower() == 'n':
        ...
        with open('./potsition_data/keyboard_pos_data.txt', 'w') as file:
            json.dump(keyboard_positions, file)

        # 取得驅動軟體上的 RGB設置 座標
        rgb_positions = get_positions('RGB setting')

        with open('./potsition_data/rgb_pos_data.txt', 'w') as file:
